...
class VideoMediaForm(forms.ModelForm):
    class Meta: 
        ...
        fields = ('author', 'title', 'description', 'language')
...
class RegisterForm(forms.Form):
    ...
    def clean(self):
        ...
        if 'email' in self.cleaned_data:
            if len(models.TravelsUser.objects.filter(email=self.cleaned_data['email'])):
